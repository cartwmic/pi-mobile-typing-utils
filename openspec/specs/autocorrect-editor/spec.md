...
The `AutocorrectEditor` component replaces Pi's default editor when autocorrect is enabled. It intercepts keystrokes to apply word-by-word corrections at trigger characters (space and select punctuation), supports backspace-to-undo within a one-character window, suppresses re-correction of recently rejected words, and surfaces correction feedback via a status line. It is also responsible for token eligibility decisions (skipping paths, identifiers, flags, etc.) so the correction engine only sees natural-language words. The editor receives a UI adapter via constructor injection — it does NOT reach into `ctx.ui` directly.
